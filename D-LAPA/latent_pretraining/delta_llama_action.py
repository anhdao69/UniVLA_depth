...
class FlaxVideoLLaMAPreTrainedModel(FlaxPreTrainedModel):
    ...
    def __call__(
        ...
        outputs = self.module.apply(
            ...
            delta_masks=jnp.array(delta_masks, dtype="f4"),
...
class FlaxDeltaActionLaMAForCausalLMModule(nn.Module):
    config: VideoLLaMAConfig
    dtype: jnp.dtype = jnp.float32
    param_dtype: jnp.dtype=jnp.float32
    precision: Optional[Union[jax.lax.Precision, str]]=None
    ...
    def __call__(
        ...
        if self.config.sample_mode == 'all':
            if not return_dict:
                return (vision_logits, lm_logits, delta_logits, action_logits) + outputs[1:]

            return FlaxCausalLMOutput(logits=(vision_logits, lm_logits, delta_logits, action_logits), hidden_states=outputs.hidden_states, attentions=outputs.attentions)
        elif self.config.sample_mode == 'vision':
            if not return_dict:
